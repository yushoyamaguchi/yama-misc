...
echo "Setting up Cilium LB (native) on Kind"
...
sudo cilium install --wait --chart-directory="${CILIUM_DIR}"/install/kubernetes/cilium --values cilium-config-native.yaml
...
sudo kubectl apply -f daemonsets.yaml
